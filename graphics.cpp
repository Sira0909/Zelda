...
class renderingWindow : public BaseWindow<renderingWindow>{
    ...
    int level=120;
    double linkxPos = 112.0; 
    double linkyPos = 80.0;
    int direction = 2;
    ...
    void drawthemap(double offx, double offy, double off2x, double off2y){
        ...
        for(int i = 1;i<42;i++){
            ...
            drawamap(levelmap.typemaps[i-1],i,offx,offy,pRenderTarget,brushlist[firstbrush(i)],brushlist[secondbrush(i)],pBlueBrush);
            drawamap(levelmap.typemaps2[i-1],i,off2x,off2y,pRenderTarget,brushlist[firstbrush(i)],brushlist[secondbrush(i)],pBlueBrush);
        }
        // for(int i = 0;i<176;i++){
    ...
    void    OnPaint(){
        ...
        if (SUCCEEDED(hr))
        {
            ...
            switch(screenmoving){
                case(0):
                    drawthemap(0,0,16,16);
                    break;
                case(1):
                    drawthemap(0,offset,0,11+offset);
                    break;
                case(2):
                    drawthemap(0,offset,0,-11+offset);
                    break;
                case(3):
                    drawthemap(offset,0,16+offset,0);
                    break;
                case(4):
                    drawthemap(offset,0,-16+offset,0);
                    break;
            ...
            // drawmap(Window(),pRenderTarget,level);

            drawlink(Window(),pRenderTarget,linkxPos,linkyPos,direction,movesprite/2);
        ...
        case WM_TIMER:
            ...
            if(screenmoving!=0){
                ...
                switch(direction){
                    case (1):
                        linkyPos-=2;
                        if(checkCollision(true,true)){linkyPos+=2;}
                        if(linkyPos<=0){level-=16;screenmoving = 1;offset =-11;levelmap.updatelevel(level);}
                        break;
                    case (2):
                        linkyPos+=2;
                        if(checkCollision(true)){linkyPos-=2;}
                        if(linkyPos>=160){level+=16;screenmoving = 2;offset =11;levelmap.updatelevel(level);}
